        ...
        {filters.map(([key, label]) => (
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
              filter === key ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
            }`}
          >
            {label}
          </button>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map((t) => (
              <tr key={t.id} className="hover:bg-slate-50">
                ...
                <td className="px-5 py-3 text-slate-900">{t.customer_name || '—'}</td>
                <td className="px-5 py-3 text-slate-600">{t.service_name}</td>
                <td className="px-5 py-3"><StatusBadge status={t.status} /></td>
                <td className="px-5 py-3 text-slate-600">{t.counter_name || '—'}</td>
                <td className="px-5 py-3 text-slate-600">{t.attendant_name || '—'}</td>
                <td className="px-5 py-3 tabular-nums text-slate-600">{fmtTime(t.created_at)}</td>
                <td className="px-5 py-3 tabular-nums text-slate-600">{fmtTime(t.called_at)}</td>
