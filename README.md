...
- **Permissões por usuário**: ao criar/editar um usuário, o superusuário marca quais telas ele acessa (Atendimento, Consultório, Gestão de Senhas, Dashboard, Relatórios) e também os atalhos das telas públicas (Abrir Totem, Abrir Painel TV). O menu e as rotas obedecem às permissões; Configurações é sempre exclusiva do superusuário.
- **Perfis de acesso**: em *Configurações → Usuários* é possível criar perfis nomeados (conjuntos de permissões reutilizáveis — ex.: Recepção, Médico, Gestão). Ao cadastrar um usuário, basta escolher o perfil para aplicar as permissões de uma vez (ainda editáveis). Vêm quatro perfis prontos por padrão.
...
- **Fluxo médico opcional (Configurações → Fluxo Médico)**: quando habilitado, a mesma senha passa por dois estágios — recepção (triagem) e médico (consultório) — sem o paciente pegar duas senhas. Desligado (padrão), o sistema opera só com a recepção.
  - **Recepção**: o atendente chama a senha, faz a triagem e pode *Encaminhar para o médico* (escolhendo a especialidade) **ou** *Finalizar no guichê* — nem todo atendimento precisa do médico. A senha encaminhada entra na fila do médico mantendo o mesmo código.
  - **Médicos online**: o sistema identifica em tempo real quais médicos estão logados (tela de Consultório aberta). A recepção tem um card *Médicos* com o status online/offline **por especialidade e por profissional**, e o seletor de especialidade mostra 🟢 online / 🔴 offline. O encaminhamento para uma especialidade sem médico online é bloqueado com aviso (com opção de encaminhar mesmo assim, se desejado) — evitando senha presa sem ninguém para atender.
  ...
  - **Relatório por médico**: com o fluxo ligado, a tela de Relatórios ganha a tabela *Atendimento por médico* (pacientes chamados/concluídos, espera média na fila médica, consulta média e tempo total em consultas por médico).
  - **Perfis de usuário**: a lista de usuários mostra o tipo de login (Superusuário, Médico, Atendente, Gestão) com cores e permite filtrar por tipo. Usuários de perfil médico não veem os atalhos de Totem/Painel TV no menu.
